/**
 * Created by 슈이롱 on 2016-06-09.
 * To check personal identify card
 */
(function ($) {
    var Config = {
    ...
    $.fn.CheckIdCard = function (option) {
        var defaultOption = {
            success: function () {
                alert("您的身份证号已通过验证。");
                return true;
            ...
            failure: function () {
                alert("为了办理正式通关材料，需提供您的真实姓名和身份证号码！");
                return false;
            },
            checklength: function () {
                alert("身份证号码须为18位或15位数字.最后一位可以是大小写字母X.");
                return false;
            }
        };
        ...
        var fun = {
                eighteen: function (sIdCard) {//when id number length equals to eighteen
                    ...
                    return (1900 <= year && year <= nowDay.getFullYear() - 18 && month == (checkDay.getMonth() + 1) && day == checkDay.getDate()) ? true : false;
                },
                fifteen: function (sIdCard) { //when id number length equals to fifteen
                    var year = parseFloat("19" + sIdCard.substr(6, 2));
                    var month = parseFloat(sIdCard.substr(8, 2));
                    var day = parseFloat(sIdCard.substr(10, 2));
                    var checkDay = new Date(year, month - 1, day);
                    //after 1999 the id card number changed to eighteen of length
                    return (1900 <= year && year < 2000 && month == (checkDay.getMonth() + 1) && day == checkDay.getDate()) ? true : false;
                },
                validate: function (sIdCard) {//check last number when the length is 18 by algorithm
                    var aIdCard = sIdCard.split("");
                    var sum = 0;
                    for (var i = 0; i < Config.Wi.length; i++) {
		...
		if($(this).attr('ignore')==1){
				...
				if (sIdCard.match(/^\d{15}|\d{17}[0-9xX]{1}$/gi) == null) {//to check id number length that is 18 or 15 and it is possible  to be a "X" at last letter when the length is 18. 
					...
					case 18:
						(fun.province(sIdCard) && fun.eighteen(sIdCard) && fun.validate(sIdCard)) ? opt.success() : opt.failure();
						break;
					case 15:
						(fun.province(sIdCard) && fun.fifteen(sIdCard)) ? opt.success() : opt.failure();
						break;
					default:
						opt.checklength();
						break;
